...
import { AdminAttemptSummary, CandidateAttemptService } from '../../services/candidate-attempt.service';
import {
  AssignmentsService,
  AssignmentSummary,
...
export class CandidateScorePageComponent implements OnInit {
  ...
  get averageScore(): number | null {
    const scored = this.rows.map((item) => item.scoreFinal).filter((value): value is number => value !== null);
    if (scored.length === 0) {
      return null;
    }
  ...
  private mapRows(
    ...
    const rowsFromAttempts = attempts
      .filter((attempt) => {
        ...
        return attempt.assignmentId !== null && candidateAssignments.some((item) => item.id === attempt.assignmentId);
      })
      .map((attempt) => {
        const assignment =
        ...
        return {
          ...
          testTitle:
            attempt.testTitle ??
            (testId === null ? '-' : test?.title ?? `Prueba #${testId}`),
